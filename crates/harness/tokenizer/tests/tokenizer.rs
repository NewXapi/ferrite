...
use harness_tokenizer::engine::{BYTES_PER_TOKEN, TokenizerEngine};
...
fn test_guesstimate_calculations() {
    ...
    assert_eq!(
        TokenizerEngine::guesstimate("abcdefghijklmnopqrstuvwxyz"),
        8
    ); // 26 bytes / 3.35 = 7.76 -> ceil = 8
...
fn test_invalid_json_and_missing_file() {
    ...
    assert!(matches!(
        err,
        TokenModelError::Io { .. } | TokenModelError::UnsupportedFormat { .. }
    ));
    ...
    assert!(matches!(
        err,
        TokenModelError::Json { .. } | TokenModelError::UnsupportedFormat { .. }
    ));
}
...
fn test_registry_operations() {
    ...
    assert!(matches!(
        registry.get("guesstimate"),
        Some(TokenizerEngine::Guesstimate)
    ));

    // Test from_dir with empty directory.
    let temp_dir = tempfile::tempdir().expect("failed to create temp dir");
    let (reg, errors) =
        TokenizerRegistry::from_dir(temp_dir.path()).expect("failed to load registry");
    ...
    reg2.register(
        "test".to_string(),
        TokenizerEngine::HuggingFace(
            tokenizers::Tokenizer::from_file("nonexistent.json").unwrap_or_else(|_| {
