...
pub fn UserCard(
    ...
    let panel_basic = rsx! {
        div { class: "space-y-2.5",
            div { class: "flex justify-between gap-2 {crate::T_text_xs}",
                span { class: "{crate::T_text_zinc_400}", "用户名" }
                span { class: "{crate::T_font_medium} {crate::T_text_zinc_200} truncate", "{user.username}" }
            }
            div { class: "flex justify-between gap-2 {crate::T_text_xs}",
                span { class: "{crate::T_text_zinc_400}", "邮箱" }
                span { class: "{crate::T_font_medium} {crate::T_text_zinc_200} truncate", "{user.email}" }
            }
            div { class: "flex justify-between gap-2 {crate::T_text_xs}",
                span { class: "{crate::T_text_zinc_400}", "角色" }
                span { class: "{crate::T_font_medium} {crate::T_text_zinc_200}", "{role_str}" }
            }
            div { class: "flex justify-between gap-2 {crate::T_text_xs}",
                span { class: "{crate::T_text_zinc_400}", "状态" }
                span { class: "{crate::T_font_medium} {crate::T_text_zinc_200}", "{status_str}" }
            }
            div { class: "space-y-1.5",
                p { class: "{crate::T_text_11px} {crate::T_text_zinc_400}", "分组" }
                div { class: "flex flex-wrap gap-1.5",
                    for label in &group_labels {
                        span {
                            class: "rounded-full border {crate::T_border_zinc_700} bg-zinc-800/80 px-2 py-0.5 {crate::T_text_11px} {crate::T_text_zinc_300}",
                            "{label}"
                        }
                    }
                    if group_labels.is_empty() {
                        span { class: "{crate::T_text_11px} {crate::T_text_zinc_500}", "无分组" }
                    }
                }
            }
    ...
    let panel_quota = rsx! {
        div { class: "space-y-2",
            div { class: "flex justify-between gap-2 {crate::T_text_xs}",
                span { class: "{crate::T_text_zinc_400}", "已用" }
                span { class: "{crate::T_font_medium} {crate::T_text_zinc_200}", "{fmt_quota_cny(user.used_quota)}" }
            }
            div { class: "flex justify-between gap-2 {crate::T_text_xs}",
                span { class: "{crate::T_text_zinc_400}", "总额" }
                span { class: "{crate::T_font_medium} {crate::T_text_zinc_200}", "{fmt_quota_cny(user.quota)}" }
            }
            div { class: "h-1.5 w-full overflow-hidden rounded-full {crate::T_bg_zinc_800}",
                div { class: "h-full rounded-full {usage_tone(used_pct)} transition-all", style: "width: {used_pct:.1}%" }
            }
            div { class: "flex justify-between gap-2 {crate::T_text_xs}",
                span { class: "{crate::T_text_zinc_400}", "请求数" }
                span { class: "{crate::T_font_medium} {crate::T_text_zinc_200}", "{user.request_count}" }
            }
        }
    };
    let panel_system = rsx! {
        div { class: "space-y-2 {crate::T_text_xs}",
            div { class: "flex justify-between gap-2",
                span { class: "{crate::T_text_zinc_400}", "Key" }
                span { class: "font-mono {crate::T_text_zinc_200}", "{short_k}" }
            }
            div { class: "flex justify-between gap-2",
                span { class: "{crate::T_text_zinc_400}", "创建" }
                span { class: "{crate::T_text_zinc_200}", "{user.created_at}" }
            }
        }
    };
