...
///
/// The tabs show the CNY-denominated overview, redemption information,
/// generation time, and system key. `status` follows the billing API exactly:
/// `1` is unused, `2` is redeemed, and `3` is disabled; other values render
/// as "未知状态". `quota_cny` is an already converted CNY amount and is shown
/// with two decimal places. This card intentionally has no edit affordance:
/// future operations must be connected through a Popover or status workflow.
#[component]
                    ...
                    1 => rsx! {
                        div { class: "space-y-2 text-xs",
                            div { class: "flex justify-between gap-2",
                                ...
                                span { class: "text-zinc-200", "{redeemed_by.clone().unwrap_or_else(|| \"无\".to_string())}" }
                            }
                            div { class: "flex justify-between gap-2",
                                span { class: "text-zinc-400", "核销时间" }
                                span { class: "text-zinc-200", "{redeemed_at.clone().unwrap_or_else(|| \"无\".to_string())}" }
                            }
                        }
                    },
                    2 => rsx! {
