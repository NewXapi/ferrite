//! Avatar — rust-ui (github.com/rust-ui/ui) registry 原版拷贝。
//!
//! 与 crate 内 components/avatar 的差别：children 组合式 API（Avatar/AvatarImage/
//! AvatarFallback/AvatarBadge），尺寸走 data-[size] 档位 + tw_merge 合类；
//! 仅去掉上游 strum 依赖（as_str 手写）。供 AvatarMenu 复用。
use dioxus::prelude::*;
