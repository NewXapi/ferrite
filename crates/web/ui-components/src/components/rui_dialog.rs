...
pub fn DialogContent(
    ...
    /// 受控开关：与 `on_close` 搭配（条件挂载的弹窗挂载即打开）。
    /// `on_close` 为 Some 时完全由 Dioxus 驱动开合、跳过 JS 脚本接线；
    /// None 保持上游触发器模式（初始 closed，点触发器打开）。
    #[props(default)]
    open: bool,
    ...
    // 受控模式：状态来自 props，脚本不接线（触发器/ESC 不参与），
    // 关闭一律走 on_close（X、backdrop、取消/成功回调）。
    let controlled = on_close.is_some();
    let state = if open { "open" } else { "closed" };
    let pointer_events = if open { "auto" } else { "none" };

    // 非受控模式沿用上游 JS 接线（打开=触发器点击、关闭=[data-dialog-close]）。
    let backdrop_behavior = if close_on_backdrop_click && !controlled {
        "auto"
    } else {
        "manual"
    };
