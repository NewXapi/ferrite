...
/// - `user_name`：登录用户名；None 时 footer 显示「登录」链接。
#[component]
pub fn SectionRail(
    active_index: usize,
    ...
    rsx! {
        aside {
            ...
            // account footer：登录显示首字符圆点 + 名字；未登录给登录链接
            div {
                ...
                match user_name {
                    Some(name) => rsx! {
                        div {
                            ...
                            title: "{name}",
                            span {
                                class: "flex h-8 w-8 items-center justify-center rounded-full border border-zinc-700 bg-zinc-800 text-xs font-semibold text-zinc-200",
                                "{name.chars().next().unwrap_or('?')}"
                            }
                            span { class: "max-w-12 truncate text-[10px] text-zinc-500", "{name}" }
                        }
                    },
                    None => rsx! {
                        a {
                            class: "flex h-8 w-8 items-center justify-center rounded-lg text-zinc-500 hover:bg-zinc-900 hover:text-zinc-200",
                            href: "#signup",
                            aria_label: "登录",
                            "+"
                        }
