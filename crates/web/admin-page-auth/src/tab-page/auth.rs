//! Auth page: login / sign-up orchestration layer.
//!
//! `tab-page/` 只做编排：hash → tab 切换、provide 共享提交状态、挂载
//! `components::auth_view::AuthPage`。真正的 UI 组件与 wire 在
//! `components/` 与 `api.rs`。

use crate::components::auth_form::SubmitState;
use crate::components::auth_state::use_auth_tab;
use crate::components::auth_view::AuthPage;
use dioxus::prelude::*;
...
pub fn AuthPageRoot() -> Element {
    ...
    let _listener = use_signal(|| {
        ...
        let to_tab = |h: &str| {
            if h == "#signup" {
                crate::components::auth_state::AuthTab::SignUp
            } else {
                crate::components::auth_state::AuthTab::SignIn
            }
        };
