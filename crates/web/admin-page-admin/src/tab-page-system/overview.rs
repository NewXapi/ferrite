...
/// (区段头左 `text-lg font-medium text-foreground` 标题 + 右描边按钮);统计卡
/// 网格 `grid grid-cols-1 gap-3 md:grid-cols-3 lg:grid-cols-5`(手机 1 / 中屏 3
/// / 大屏 5 列);`section#system-sec-counts` 同网格但仅在有数据时渲染;
/// `section#system-sec-env` 为 `rounded-xl border border-border bg-card/60
/// p-5`,明细行 `divide-y divide-zinc-800/80` + 左标签 `text-muted-foreground` / 右值
/// `font-mono text-foreground`。错误/加载/空态均为虚线或红边圆角块 + 居中文字。
///
                ...
                div { class: "divide-y divide-zinc-800/80",
                    for (label, value) in env_rows {
                        div { class: "flex items-center justify-between gap-4 py-2.5",
                            ...
                            span { class: "break-all text-right text-xs font-mono text-foreground", "{value}" }
                        }
                    }
                }
            }
