...
use super::gateway_health_shared::{LBL_REMAINING_PREFIX, MSG_CHANNEL_FALLBACK_PREFIX, MSG_CHANNEL_FALLBACK_SUFFIX, MSG_MODEL_FALLBACK};
...
fn display_name(item: &GatewayHealthItem) -> String {
        ...
        && !name.is_empty()
    {
        ...
        let fallback = item
            ...
            .take(8)
            .collect::<String>();
        format!("{MSG_CHANNEL_FALLBACK_PREFIX}{fallback}{MSG_CHANNEL_FALLBACK_SUFFIX}")
    }
}
...
fn state_tone(state: HealthItemState) -> &'static str {
    use super::gateway_health_shared::{TONE_COOLING, TONE_SLOW_START, TONE_OK};
    match state {
