...
/// border border-border bg-card/60 p-6 space-y-6`;两条通路用
/// `border-b border-border pb-6` 分隔,标题 h3 `text-sm font-medium
/// text-foreground`;输入框统一 `bg-background border border-border rounded-xl`
/// + `placeholder-zinc-500 focus:border-blue-500`;导入按钮 `bg-blue-600
/// hover:bg-blue-500 disabled:bg-accent`;结果区错误红底 / 报告绿底
/// (`border-emerald-500/30 bg-success`),失败明细行左红边框。
///
    ...
    rsx! {
        section {
            ...
            div { class: "space-y-4 border-b border-border pb-6",
                ...
                div { class: "space-y-3",
                    input {
                        class: "w-full bg-background border border-border rounded-xl px-4 py-3 {ui::TYPE_BODY} placeholder-zinc-500 focus:outline-none focus:border-blue-500",
                    ...
                    button {
                        class: "w-full px-6 py-3 bg-blue-600 hover:bg-blue-500 disabled:bg-accent rounded-xl {ui::TYPE_CARD_TITLE} transition-colors",
            ...
            div { class: "space-y-4",
                ...
                textarea {
                    class: "w-full h-32 bg-background border border-border rounded-xl px-4 py-3 text-sm text-foreground placeholder-zinc-500 focus:outline-none focus:border-blue-500 font-mono resize-y",
                    placeholder: "vless://...\nvmess://...\nss://...",
                    value: share_text(),
                ...
                div { class: "grid grid-cols-2 gap-3",
                    input {
                        class: "bg-background border border-border rounded-xl px-4 py-3 {ui::TYPE_BODY} placeholder-zinc-500 focus:outline-none focus:border-blue-500",
                        placeholder: FIELD_CHANNELS_PLACEHOLDER,
                        value: channels_str(),
                        oninput: move |e| channels_str.set(e.value()),
                    }
            ...
            if let Some(e) = err_msg() {
                ...
                div {
                    role: "status",
                    class: "rounded-xl border border-emerald-500/30 bg-success p-5",
                    ...
                    if !r.failures.is_empty() {
                        div { class: "mt-3 pt-3 border-t border-border",
                            ...
                            for f in &r.failures {
                                div {
                                    class: "mb-2 {ui::TYPE_DESC} p-3 bg-background rounded border-l-4 {ui::B_DANGER}",
                                    "data-testid": "proxy-import-failure",
                                    span { class: "font-mono {ui::C_DANGER}", "{f.source}" }
                                    span { class: "{ui::C_MUTED} mx-2", "→" }
                                    span { class: "text-foreground", "{f.reason}" }
                                }
                            }
                        }
                    } else {
