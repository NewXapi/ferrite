...
pub fn GroupInspect(index: usize) -> Element {
    ...
    let aliases: Vec<String> = edges
        ...
        .collect();
    let Some(r) = row else {
        return rsx! { p { class: "{ui::TYPE_DESC}", {MSG_GROUP_ABSENT} } };
    };
...
pub fn AliasInspect(index: usize) -> Element {
    ...
    let dispatch: Vec<String> = edges
        ...
        .collect();
    let Some(r) = row else {
        return rsx! { p { class: "{ui::TYPE_DESC}", {MSG_ALIAS_ABSENT} } };
    };
...
fn InspectList(title: &'static str, items: Vec<String>, empty: &'static str) -> Element {
    rsx! {
        div { class: "space-y-1.5",
            span { class: "{ui::TYPE_LABEL}", "{title}" }
            if items.is_empty() {
                p { class: "{ui::TYPE_LABEL}", "{empty}" }
            } else {
