...
//! [`crate::components::GatewayHealthShared`]。
//!
//! 渲染约定(仓库铁律):
...
//!
//! 边界:文案常量与语义色在 `components/gateway_health_shared`,
//! 单行渲染在 `components/gateway_health_row`;本文件不发写请求
//! (本 tab 只读),也不做失败重试策略(交给用户点按钮)。

use client::{fetch_gateway_health, ApiClient, GatewayHealthItem, HealthItemState};
use dioxus::prelude::*;

use crate::components::{
    GatewayHealthRow, GatewayHealthToolbarSection, GatewayHealthSection,
};
use crate::shared::{
    BTN_REFRESH, BTN_RETRY, LBL_COUNT_COOLING_PREFIX, LBL_COUNT_OK_PREFIX,
    LBL_COUNT_SLOW_PREFIX, LBL_POLLING, LBL_SYNCED, SEC_EMPTY_HINT,
    SEC_EMPTY_NOTE, SEC_LOAD_FAILED, SEC_PANEL,
};
use crate::state::{GatewayHealthItem as StateGatewayHealthItem, parse_gateway_health};
...
/// [`GatewayHealthRow`]。不负责单行内部的回退与取色(在 `components/gateway_health_row`),
/// 不做写操作(本 tab 只读)。
///
/// 【交互逻辑】用户操作 → 组件行为 → 数据交互:
/// - 挂载 → `use_effect`(依赖 `reload`)启动轮询循环,循环内先拉一次快照,
///   若存在 cooling / slow_start 条目则 `TimeoutFuture` 等 5s 后继续,
