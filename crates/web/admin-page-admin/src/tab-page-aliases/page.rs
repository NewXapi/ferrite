    ...
    rsx! {
        div { class: "flex flex-col gap-6",
            ...
            if let Some(msg) = notice() {
                div {
                    role: "status",
                    class: "rounded-xl border border-zinc-700 bg-zinc-900 px-4 py-2 {ui::TYPE_DESC}",
                    "{msg}"
                    if busy() { " ···" }
                }
            }
            ...
            div { class: "flex flex-wrap items-center gap-2 rounded-xl border border-zinc-700/60 bg-zinc-900/60 px-4 py-2.5 {ui::TYPE_DESC}",
                span { "{SEC_DATA_NOTE}" }
            }
