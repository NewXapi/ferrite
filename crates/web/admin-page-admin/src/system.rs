...
//! 以及真实 `/api/option`(admin-ops options)的站点选项区。
//! 包含:顶部运行指标概览(运行时长/内存/CPU/数据库/进程内存)、核心实体统计、
//! 运行环境明细行、站点选项(key/value 平表,真实读写)。
//! 所有数据来自后端实时采集;后端不提供的站点配置/功能开关字段已移除,不再
//! 使用 EntityStore mock 假数据。
//!
//! 下方还挂载出口代理节点相关面板:导入(消费 `/api/proxy_nodes/subscription`
//! 与 `/batch`)与运行态(消费 `/api/proxy_nodes/report`,呈现网关数据面
//! 每节点的 inflight/失败计数/冷却剩余/最近延迟)。
