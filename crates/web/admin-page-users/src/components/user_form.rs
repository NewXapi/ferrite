    ...
    rsx! {
        Modal { title: title.to_string(), on_close: move |_| on_cancel.call(()),
            ...
            if tab() == FormTab::Basic {
                div { class: "space-y-4",
                    div {
                        label { class: "mb-1.5 block {ui::T_text_xs} {ui::T_text_zinc_400}", "{FIELD_USERNAME}" }
                        input {
                            class: MODAL_INPUT,
                    ...
                    if !editing {
                        div {
                            label { class: "mb-1.5 block {ui::T_text_xs} {ui::T_text_zinc_400}", "{FIELD_INIT_PASSWORD}" }
                            input {
                                class: MODAL_INPUT,
                                r#type: "password",
                    ...
                    div {
                        label { class: "mb-1.5 block {ui::T_text_xs} {ui::T_text_zinc_400}", "{LBL_EMAIL}" }
                        input {
                            class: MODAL_INPUT,
                            r#type: "email",
                    ...
                    div {
                        label { class: "mb-1.5 block {ui::T_text_xs} {ui::T_text_zinc_400}", "{FIELD_ROLE}" }
                        RoleChips { role, on_change: move |v: u16| role.set(v) }
                    }
                    div {
                        label { class: "mb-1.5 block {ui::T_text_xs} {ui::T_text_zinc_400}", "{LBL_QUOTA}" }
                        input {
            ...
            if tab() == FormTab::Group {
                div { class: "space-y-4",
                    div {
                        label { class: "mb-1.5 block {ui::T_text_xs} {ui::T_text_zinc_400}", "{FIELD_GROUPS}" }
                        GroupChips { group, on_change: move |v: Vec<String>| group.set(v) }
                        p { class: "mt-1 {ui::TYPE_DESC}", "{FIELD_GROUP_HINT}" }
                    }
                    div {
                        label { class: "mb-1.5 block {ui::T_text_xs} {ui::T_text_zinc_400}", "{FIELD_NOTE}" }
                        textarea {
            ...
            if tab() == FormTab::Binding {
                div { class: "rounded-xl border {ui::T_border_zinc_800} {ui::T_bg_zinc_950} px-4 py-3 {ui::T_text_xs}",
                    p { class: "mb-2 {ui::T_text_11px} {ui::T_text_zinc_500}", "{MSG_BINDING_READONLY}" }
                    div { class: "space-y-1.5",
                        for (label, value) in [("GitHub", "-"), ("Discord", "-"), ("OIDC", "-"), ("WeChat", "-"), ("Telegram", "-"), (LBL_EMAIL, email_bound)] {
                            div { class: "flex justify-between gap-2",
                                span { class: "{ui::T_text_zinc_400}", "{label}" }
                                span { class: "{ui::T_font_medium} {ui::T_text_zinc_200}", "{value}" }
                            }
                        }
                    }
                }
