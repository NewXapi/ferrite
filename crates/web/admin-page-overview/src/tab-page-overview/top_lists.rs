        ...
        Card {
            ...
            CardHeader {
                ...
                div { class: "flex items-center justify-between gap-3",
                    h3 { class: "{ui::TYPE_CARD_TITLE} text-foreground", "{title}" }
                    div { class: "text-right", "data-testid": "{total_testid}",
                        ...
                        p { class: "{ui::TYPE_LABEL} text-muted-foreground", "{total_label}" }
                    }
                }
            }
            CardContent {
                ...
                if rows.is_empty() {
                    p { class: "py-6 text-center {ui::TYPE_DESC} text-muted-foreground", "{TOP_EMPTY}" }
                }
                for (i, row) in rows.iter().enumerate() {
                    TopRowItem { index: i, name: row.name.clone(), amount: row.amount.clone(), growth: row.growth.clone(), share: row.share.clone() }
                }
                p { class: "pt-1 {ui::TYPE_LABEL} leading-4 text-muted-foreground/60",
                    "{TOP_FOOTNOTE}"
                }
            }
        }
...
pub fn TopRowItem(
    ...
    rsx! {
        div { class: "flex items-center gap-3 rounded-lg -mx-2 px-2 py-1.5 transition-all hover:bg-accent cursor-default",
            div { class: "flex h-5 w-5 shrink-0 items-center justify-center rounded bg-secondary/80 {ui::TYPE_LABEL} text-muted-foreground shadow-sm transition-colors hover:bg-accent hover:text-foreground", "{index + 1}" }
            div { class: "flex-1 min-w-0 flex items-center justify-between gap-3",
                span { class: "truncate {ui::TYPE_CARD_TITLE} text-foreground/80 transition-colors hover:text-foreground", "{name}" }
                div { class: "flex shrink-0 flex-col items-end gap-0.5",
                    span { class: "text-xs font-mono text-muted-foreground transition-colors hover:text-foreground/80", "{amount}" }
                    div { class: "flex items-center gap-1.5 {ui::TYPE_LABEL} leading-none",
