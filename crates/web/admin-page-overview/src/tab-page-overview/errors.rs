...
pub fn ErrorsPanel() -> Element {
    ...
    rsx! {
        Card {
            hoverable: true,
            CardHeader {
                CardTitle { class: "{ui::T_text_lg} text-foreground", "{SEC_ERRORS}" }
                CardAction {
                    div { class: "flex items-center gap-3",
                        // 合计错误数大数字 + asOf 本地时间（裸值，不写「数据截至」）
                        div { class: "text-right", "data-testid": "errors-total",
                            p { class: "{ui::T_text_xl} {ui::T_font_semibold} leading-none font-mono tabular-nums text-foreground", "{total_text}" }
                            p { class: "mt-0.5 {ui::T_text_10px} text-muted-foreground", "{ERRORS_TOTAL_LABEL}" }
                        }
                ...
                section { "data-testid": "errors-panel",
                    ...
                    if err.is_some() {
                        ...
                        div { class: "rounded-2xl border border-dashed border-border bg-card/50 py-10 text-center",
                            ...
                            p { class: "mt-1 {ui::T_text_xs} text-muted-foreground/70", "{ERRORS_EMPTY_HINT}" }
                        }
                    } else {
                        for r in items {
                            div { class: "flex items-center gap-3",
                                ...
                                span { class: "min-w-0 flex-1 truncate {ui::T_text_sm} text-foreground", "{r.model_name}" }
                                span { class: "shrink-0 {ui::T_text_xs} font-mono tabular-nums text-muted-foreground", "{r.count}" }
                                span { class: "w-12 shrink-0 text-right {ui::T_text_xs} font-mono tabular-nums text-muted-foreground/70",
