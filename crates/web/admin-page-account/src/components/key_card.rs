//! 密钥卡片 — 数据来自 TokenDto (GET /api/token)，rust-ui 组件重构（批注 28fff91a）。
//! 操作: 编辑 (改名) / 停用·启用 (status 1↔2) / 删除, 成功后由父面板刷新列表。
//! 已用额度走 $ 口径 (fmt_quota, 500_000 ≈ $1) + Progress 进度条,
//! 无限额度显示「无限」Info 徽标且不渲染进度条。
...
use ui::components::rui_progress::Progress;
...
///
/// 【做什么】负责渲染单个 TokenDto 的完整展示与交互区：名称+掩码预览、启用/停用 Badge、已用额度 ($ 口径) 与 Progress、创建时间、编辑/切换/删除三个按钮。不负责数据获取、列表刷新、弹窗弹出，仅把用户操作通过回调透传给父页面。
///
/// 【交互逻辑】
...
///
/// 【样式】rust-ui Card 承载（hover 时边框变亮）；状态徽标 Success/Warning 变体、无限额度 Info 变体；Progress 轨道 zinc-800、内条三档：≥90% 红、≥70% 黄、<70% 绿；底部分隔线 Separator，三个按钮等宽 flex-1 Ghost。
///
/// 【子组件组成】rui Card + Badge ×2 + Progress + Separator + Button ×3
///
/// 【数据流】
...
pub fn KeyCard(
    ...
    // 配色随用量升高转告警（与 admin-page-users 面板同语义）
    ...
    rsx! {
        Card { class: "gap-3 bg-zinc-900/60 px-4 py-4 transition-all duration-200 hover:border-zinc-600 hover:bg-zinc-900/80",
            div { class: "flex items-start justify-between gap-2",
                ...
                if enabled {
                    Badge { variant: BadgeVariant::Success, "启用" }
                } else {
                    Badge { variant: BadgeVariant::Warning, "停用" }
                }
            }
            ...
            div { class: "space-y-2 text-xs",
                div { class: "flex items-center justify-between gap-2",
                    ...
                    if entry.unlimited_quota {
                        Badge { variant: BadgeVariant::Info, "无限" }
                    } else {
                ...
                if !entry.unlimited_quota {
                    Progress { value: pct as f64, max: 100.0, class: "h-1.5 bg-zinc-800", bar_class: "{bar_tone}" }
                }
                if !created.is_empty() {
