            ...
            div { class: "space-y-4",
                div { class: "flex items-center justify-between gap-3",
                    div { class: "flex items-center gap-2",
                        ...
                        span { class: "{ui::T_text_xs} px-3 py-1 rounded-full {ui::T_bg_zinc_800} {ui::T_text_zinc_400}",
                ...
                if !keys_err().is_empty() {
                    p { class: "{ui::T_text_sm} {ui::STATE_WARNING_TEXT}", "无法加载密钥 (未登录或请求失败): {keys_err()}" }
                } else if !keys_loaded() {
                    p { class: "{ui::T_text_sm} {ui::T_text_zinc_500}", "加载中…" }
                } else if keys.is_empty() {
                    p { class: "{ui::T_text_sm} {ui::T_text_zinc_500}", "还没有密钥,点「✚ 新建密钥」签发第一个" }
                } else {
