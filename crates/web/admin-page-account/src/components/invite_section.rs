    ...
    rsx! {
        ...
        section { id: "rewards-sec-invite", class: "scroll-mt-8 space-y-4",
            ...
            section { class: "rounded-xl border {ui::T_border_zinc_800} {ui::T_bg_zinc_900} p-6 transition-colors hover:{ui::T_border_zinc_600}",
                h3 { class: "mb-4 {ui::T_text_sm} {ui::T_font_medium} {ui::T_text_zinc_200}", "邀请好友得奖励" }
                div { class: "flex flex-col gap-3 sm:flex-row",
                    div {
                        class: "flex-1 break-all rounded-2xl border {ui::T_border_zinc_700} {ui::T_bg_zinc_950} px-5 py-4 font-mono {ui::T_text_sm} {ui::T_text_zinc_400}",
                    ...
                    button {
                        class: "w-full shrink-0 rounded-2xl {ui::T_bg_white} px-8 py-4 {ui::T_font_medium} {ui::T_text_zinc_900} transition-colors hover:bg-amber-200 active:{ui::T_bg_amber_300} sm:w-auto",
                        onclick: copy_link,
            ...
            section {
                ...
                if !overview_err().is_empty() {
                    ...
                    for _ in 0..2 {
                        div {
                            class: "rounded-xl border {ui::T_border_zinc_800} {ui::T_bg_zinc_900} p-6",
                            "data-testid": "affiliate-skeleton",
                            div { class: "h-8 w-20 animate-pulse rounded {ui::T_bg_zinc_800}" }
                            div { class: "mt-3 h-4 w-28 animate-pulse rounded bg-zinc-800/70" }
                        }
                    }
                } else if let Some(ov) = overview() {
                    div { class: "rounded-xl border {ui::T_border_zinc_800} {ui::T_bg_zinc_900} p-6 transition-colors hover:{ui::T_border_zinc_600}",
                        p {
                            class: "text-4xl {ui::T_font_semibold} tracking-tight {ui::T_text_amber_300} tabular-nums",
                            "data-testid": "affiliate-invite-count",
                            "{fmt_num(ov.invite_count)}"
                        }
                        p { class: "mt-3 {ui::TYPE_CARD_TITLE}", "已邀人数" }
                        p { class: "mt-6 {ui::T_text_xs} leading-snug {ui::T_text_zinc_500}", "通过邀请完成注册的用户数" }
                    }
                    div { class: "rounded-xl border {ui::T_border_zinc_800} {ui::T_bg_zinc_900} p-6 transition-colors hover:{ui::T_border_zinc_600}",
                        p {
                            class: "text-4xl {ui::T_font_semibold} tracking-tight {ui::T_text_amber_300} tabular-nums",
                        ...
                        p { class: "mt-6 {ui::T_text_xs} leading-snug {ui::T_text_zinc_500}", "≈ {fmt_quota(ov.total_reward)} · 拉人奖励累计" }
                    }
                }
            }
        }
