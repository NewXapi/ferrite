        ...
        div { class: "flex flex-col gap-4", role: "region", "aria-label": "登录会话面板", "data-testid": "sessions-panel",
            div { class: "flex items-center justify-between gap-3",
                div {
                    ...
                    p { class: "mt-1 {ui::T_text_sm} {ui::T_text_zinc_500}", "当前用户全部存活设备与登录记录" }
                }
                Button {
            ...
            if let Some(m) = flash() {
                p { class: "{ui::T_text_xs} {ui::STATE_SUCCESS_TEXT}", "{m}" }
            }
            if !action_err().is_empty() {
                p { class: "{ui::T_text_xs} {ui::STATE_DANGER_TEXT}", "吊销失败: {action_err()}" }
            }

            if let Some(list) = sessions() {
                if list.is_empty() {
                    p { class: "{ui::T_text_sm} {ui::T_text_zinc_500}", "暂无存活会话" }
                } else {
                ...
                p { class: "{ui::T_text_sm} {ui::STATE_WARNING_TEXT}", "无法加载会话 (未登录或请求失败): {err()}" }
            } else {
