//! 系统诊断与综合运行指标采集服务。
//!
//! 提供系统运行环境、进程 Uptime、内存/CPU 资源占用以及数据库连通与核心实体统计。
...
impl SystemInfoService {
    ...
    async fn collect_database_and_counts(&self) -> (DatabaseInfo, EntityCounts) {
        // 测试连通性
        ...
        // 查询实体统计，失败回退为 0（防御性，防止个别表未就绪阻塞诊断 API）
        let users = sqlx::query_scalar::<_, i64>("SELECT count(*) FROM auth_users")
            .fetch_one(&self.pool)
            .await
            .unwrap_or(0);
        let channels = sqlx::query_scalar::<_, i64>("SELECT count(*) FROM api_channels")
            .fetch_one(&self.pool)
            .await
            .unwrap_or(0);
        let active_channels =
            sqlx::query_scalar::<_, i64>("SELECT count(*) FROM api_channels WHERE status = 1")
                .fetch_one(&self.pool)
                .await
                .unwrap_or(0);
        let models = sqlx::query_scalar::<_, i64>("SELECT count(*) FROM api_models")
            .fetch_one(&self.pool)
            .await
            .unwrap_or(0);
        let tokens = sqlx::query_scalar::<_, i64>("SELECT count(*) FROM api_tokens")
            .fetch_one(&self.pool)
            .await
            .unwrap_or(0);
