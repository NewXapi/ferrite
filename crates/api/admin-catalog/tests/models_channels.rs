...
async fn model_crud_full_flow() {
    ...
    let m = model_svc
        .create(
            ...
            "chat",
            "https://api.x.com/v1",
            "sk-abcdef123456789012345678901234567890123456789012345678901234567890",
    ...
    // 列表 + 搜索
    let (items, total) = model_svc
        .list(Some(&name), None, None, None, 1, 10)
        .await
        .expect("list");
    assert!(total >= 1);
...
async fn model_missing_detection() {
    ...
    let ch = channel_svc
        .create(
            ...
            "https://api.x.com/v1",
            vec!["sk-123".into()],
            serde_json::json!([{"alias": "llama-3", "upstream": "ollama/llama3"}]),
            "default",
            0,
            0,
            None,
    ...
    assert!(
        missing.iter().any(|m| m == "llama-3"),
        "expected llama-3 in missing, got {:?}",
        missing
    );

    // 清理
    channel_svc
        .delete(Uuid::parse_str(&ch.key).unwrap())
        .await
        .ok();
}
...
async fn channel_tag_batch_operations() {
    ...
    let disabled = channel_svc
        .batch_disable_by_tag(tag)
        .await
        .expect("batch disable");
    assert_eq!(disabled, 3);

    let enabled = channel_svc
        .batch_enable_by_tag(tag)
        .await
        .expect("batch enable");
    assert_eq!(enabled, 3);
...
async fn channel_probe_writes_history() {
    ...
    let ch = channel_svc
        .create(
            ...
            "openai",
            "https://api.x.com/v1",
            vec!["sk-12345678901234567890123456789012".into()],
            models_json(),
            "default",
            0,
            0,
    ...
    let result = channel_svc
        .test_channel(&monitor, Uuid::parse_str(&ch.key).unwrap(), None)
        .await;
    assert!(
        result.is_ok(),
        "test_channel should always Ok and record outcome"
    );
    ...
    let h = monitor
        .history(Uuid::parse_str(&ch.key).unwrap(), 10)
        .await
        .expect("history");
...
async fn channel_models_list() {
    ...
    for m in to_delete {
        model_svc
            .delete(Uuid::parse_str(&m.key).unwrap())
            .await
            .ok();
    }
}
