...
pub trait ChannelStore: Send + Sync {
    ...
    ) -> impl Future<Output = Result<MutationId, StoreError>> + Send;
    fn get_channel(
        &self,
        key: &str,
    ) -> impl Future<Output = Result<ChannelRecord, StoreError>> + Send;
    fn delete_channel(
        &self,
        key: &str,
    ) -> impl Future<Output = Result<MutationId, StoreError>> + Send;
...
pub trait RouteUnitStore: Send + Sync {
    fn put_route_unit(
        &self,
        record: &RouteUnitRecord,
    ) -> impl Future<Output = Result<MutationId, StoreError>> + Send;
    fn get_route_unit(
        &self,
        key: &str,
    ) -> impl Future<Output = Result<RouteUnitRecord, StoreError>> + Send;
    fn delete_route_unit(
        &self,
        key: &str,
    ) -> impl Future<Output = Result<MutationId, StoreError>> + Send;
}
...
pub trait UsageStore: Send + Sync {
    ...
    fn pending_usage(
        &self,
        limit: usize,
    ) -> impl Future<Output = Result<Vec<contract::mutations::Mutation>, StoreError>> + Send;
}
