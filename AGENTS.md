...
| 要派一件具体的事给某个 agent（写任务书） | 开发任务用 `.agent/tasks/dev.md`，收尾合并用 `.agent/tasks/closeout-pr.md`，web 页面 crate 重构用 `.agent/tasks/restructure-web-page-crate.md`；空白模板 `.agent/tasks/TEMPLATE.md` |
| 要重构 `crates/web/*` 页面 crate（平铺迁目录） | 规范细则 `.agent/tasks/web-page-crate-spec.md`；推进状态 `todo/web-page-refactor-rollout.md` |
