...
- **所有启停、灌数据、体检操作都走 `justfile` 里的配方**，不要自己拼命令。
...
| 6. 启动前端 | `just dev-web 8090`（用持久后台任务） | 启动后确认 8090 在监听；`curl http://127.0.0.1:8090/` 应该返回 200 |
| 7. 免登录调前端 | `just dev-web 8090 debug` | 自动登录测试账号 `admin_dev`。想手动测登录页就打开 `#login` 锚点；自己点"退出登录"不会被自动重登顶掉 |
