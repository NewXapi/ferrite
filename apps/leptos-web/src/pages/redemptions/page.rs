...
use super::data::{RedemptionCardData, RedemptionDemoData, demo_data};
        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
            ...
            <div class="rounded-xl border border-zinc-800 bg-zinc-900/60 p-4 transition-all duration-200 hover:border-zinc-700 hover:bg-zinc-900/80">
                <div class="flex items-center gap-3 mb-2">
                    ...
                    <span class="text-sm font-medium text-zinc-400">"兑换码总数"</span>
                </div>
                <div class="text-2xl font-bold text-zinc-100">{data.total_count}</div>
                <div class="text-xs text-zinc-500 mt-1">"所有兑换码的总量"</div>
            </div>
            ...
            <div class="rounded-xl border border-zinc-800 bg-zinc-900/60 p-4 transition-all duration-200 hover:border-zinc-700 hover:bg-zinc-900/80">
                <div class="flex items-center gap-3 mb-2">
                    ...
                    <span class="text-sm font-medium text-zinc-400">"未使用"</span>
                </div>
                <div class="text-2xl font-bold text-emerald-400">{data.unused_count}</div>
                <div class="text-xs text-zinc-500 mt-1">"可用于兑换的码数"</div>
            </div>
            ...
            <div class="rounded-xl border border-zinc-800 bg-zinc-900/60 p-4 transition-all duration-200 hover:border-zinc-700 hover:bg-zinc-900/80">
                <div class="flex items-center gap-3 mb-2">
                    ...
                    <span class="text-sm font-medium text-zinc-400">"已核销"</span>
                </div>
                <div class="text-2xl font-bold text-zinc-300">{data.used_count}</div>
                <div class="text-xs text-zinc-500 mt-1">"已被使用的兑换码数"</div>
            </div>
            ...
            <div class="rounded-xl border border-zinc-800 bg-zinc-900/60 p-4 transition-all duration-200 hover:border-zinc-700 hover:bg-zinc-900/80">
                <div class="flex items-center gap-3 mb-2">
                    ...
                    <span class="text-sm font-medium text-zinc-400">"已停用"</span>
                </div>
                <div class="text-2xl font-bold text-amber-400">{data.disabled_count}</div>
                <div class="text-xs text-zinc-500 mt-1">"已被停用的兑换码数"</div>
            </div>
        </div>
    }
}
...
fn RedemptionsListSection(
    ...
    view! {
        <section id="reds-sec-list" class="scroll-mt-8 space-y-4" data-testid="redemptions-list" role="list" aria-label="兑换码列表">
            // Header
            <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <div class="flex flex-col gap-1">
                    <h2 class="text-lg font-medium text-zinc-100">"兑换码列表"</h2>
                    <p class="text-sm text-zinc-500">{move || format!("{} 张卡片", filtered_count.get())}</p>
                </div>
            </div>

            // Search and filter toolbar
            <div class="rounded-xl border border-zinc-800 bg-zinc-900/60 p-4" aria-label="兑换码筛选与操作">
                <div class="flex flex-col gap-3 md:flex-row md:items-center">
                    ...
                    <div class="flex gap-2">
                        <button
                            class=move || format!("px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 {}", if status_filter.get() == 0 { "bg-emerald-500/20 text-emerald-300 border border-emerald-500/30" } else { "bg-zinc-800/80 text-zinc-300 border border-zinc-700 hover:bg-zinc-700/50" })
                            on:click=move |_| set_status_filter.set(0u8)
                        >
                            "全部"
                        </button>
                        <button
                            class=move || format!("px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 {}", if status_filter.get() == 1 { "bg-emerald-500/20 text-emerald-300 border border-emerald-500/30" } else { "bg-zinc-800/80 text-zinc-300 border border-zinc-700 hover:bg-zinc-700/50" })
                            on:click=move |_| set_status_filter.set(1u8)
                        >
                            "未使用"
                        </button>
                        <button
                            class=move || format!("px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 {}", if status_filter.get() == 2 { "bg-zinc-400/20 text-zinc-300 border border-zinc-400/30" } else { "bg-zinc-800/80 text-zinc-300 border border-zinc-700 hover:bg-zinc-700/50" })
                            on:click=move |_| set_status_filter.set(2u8)
                        >
                            "已核销"
                        </button>
                        <button
                            class=move || format!("px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 {}", if status_filter.get() == 3 { "bg-amber-500/20 text-amber-300 border border-amber-500/30" } else { "bg-zinc-800/80 text-zinc-300 border border-zinc-700 hover:bg-zinc-700/50" })
            ...
            {move || {
                ...
                if filtered.is_empty() {
                    view! {
                        <div class="rounded-2xl border border-dashed border-zinc-700 bg-zinc-900/50 py-16" role="alert" aria-label="兑换码加载失败">
                            <div class="flex flex-col items-center gap-4 text-center">
                                ...
                                <p class="text-lg font-medium text-zinc-400">"没有匹配的兑换码"</p>
                                <p class="text-sm text-zinc-500">"尝试调整搜索词或状态筛选器"</p>
                            </div>
                        </div>
                    }
                        .into_any()
...
pub fn RedemptionsPage() -> impl IntoView {
    // State management
    let redemptions = RwSignal::new(demo_data().redemptions);
    ...
    Effect::new(move |_| {
        let data = demo_data();
        redemptions.set(data.redemptions);
        loading.set(false);
    });
    ...
    view! {
        <div class="flex flex-col gap-6 p-4 md:gap-8 md:p-6">
            // Stats section
            <RedemptionsStatsSection data=stats_data.get() />

            // List section
            <RedemptionsListSection
                redemptions=redemptions.get().clone()
                on_disable=Callback::new(handle_disable)
            />
        </div>
    }
}
