...
# 1. 按序起标注栈 + 前端（详见 .agent/skills/ainotation-web/SKILL.md）
just aino-service         # MCP service（幂等）
just aino-bridge          # 同步桥 :44090（端口非 8090 时加 AINO_ORIGIN=...）
just dev-web 8090 debug   # 前端免登录预览
just aino-check           # 体检
# 2. 浏览器打开 http://127.0.0.1:8090 ，右下角点浮动 A 按钮
# 3. 标注：
