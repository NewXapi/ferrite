...
/// 起本地 mock OpenAI 上游：绑 `127.0.0.1:0`，每个连接按 `code` 回固定
/// JSON（200 → `chat.completion`；其它 → OpenAI 风格错误体），并置位命中标记。
/// 零外网，进程内自足。
///
/// 返回 `(端口, 命中标记)`——命中标记用于断言「回包确实走过上游」，
/// 而非路由层凭空造出 200。
fn spawn_mock_upstream() -> (u16, Arc<AtomicBool>) {
    spawn_mock_upstream_with(200)
}

fn spawn_mock_upstream_with(code: u16) -> (u16, Arc<AtomicBool>) {
    ...
    std::thread::spawn(move || {
        for stream in listener.incoming() {
            ...
            std::thread::spawn(move || {
                ...
                // 命中 = 请求已到达：在读到请求时立即置位（若放在回包之后，
                // 网关可从内核缓冲完成全链路，测试先于本线程置位而竞态失败）。
                thread_hit.store(true, Ordering::SeqCst);
                let (reason, body) = match code {
                    200 => ("OK", r#"{"id":"mock-1","object":"chat.completion","model":"mock-gpt","choices":[{"index":0,"message":{"role":"assistant","content":"pong"},"finish_reason":"stop"}],"usage":{"prompt_tokens":1,"completion_tokens":1,"total_tokens":2}}"#.to_string()),
                    _ => (
                        "Not Found",
                        r#"{"error":{"message":"no such model on this channel","type":"invalid_request_error","param":null,"code":"model_not_found"}}"#.to_string(),
                    ),
                };
                let resp = format!(
...
/// 文件名必须每次唯一：同进程并发跑的多个测试共用 pid 路径会互相覆写配置。
...
///
/// 端到端钉住 P1-B 全链路（egress 404 → channel_scoped → stage
/// FatalButSwitchable → retry 换候选）。400/413/422 请求相关 4xx 不换渠道、
/// 最后一个候选才透传等循环形状见 dispatch/tests/channel_downgrade.rs。
#[tokio::test]
async fn p0_channel_scoped_404_downgrades_to_second_channel() {
    // A 先被选中（priority 分层, 数字越大越优先）但回 404；B 回 200。
    let (port_a, hit_a) = spawn_mock_upstream_with(404);
    let (port_b, hit_b) = spawn_mock_upstream();
    let cfg = load_toml(&format!(
        r#"
[[keys]]
key = "sk-p0-test"
...
"#,
    ...
    // 降层成功的三件套：客户端 200（且是 B 的回包）、A/B 各收到一次请求。
    assert_eq!(
        status,
        reqwest::StatusCode::OK,
        "渠道相关 404 应降层到 B 拿 200，got {status}: {body_text}"
    );
    assert!(
        body_text.contains("pong"),
        "最终回包必须来自 B 的 mock 响应，got {body_text}"
    );
    assert!(
        hit_a.load(Ordering::SeqCst),
        "A 必须实际收到过请求（证明发生了降层换候选，而非 A 未被选中）"
    );
    assert!(
        hit_b.load(Ordering::SeqCst),
        "B 必须收到请求（200 来自 B，不是路由层幻觉）"
    );
